...
def makeContract(contractTuple):
    #('SYM','TYPE','EXCH','CURR','EXP','STRIKE','RIGHT',100)
    newContract = Contract()
    newContract.m_symbol = contractTuple[0]     #'AAPL'
    newContract.m_secType = contractTuple[1]    #'STK','OPT'
    newContract.m_exchange = contractTuple[2]   #'SMART'
    newContract.m_currency = contractTuple[3]   #'USD'
    newContract.m_expiry = contractTuple[4]     #'year/month/date'
    newContract.m_strike = contractTuple[5]     # int
    newContract.m_right = contractTuple[6]      #'CALL','PUT'
    if len(contractTuple) > 7:
        if contractTuple[1] == "OPT":
            newContract.m_multiplier = contractTuple[7]     #100
    return newContract
...
class LiveFeed(barfeed.BaseBarFeed):
    ...
    def __init__(self, identifiers, frequency, host="localhost", port=7497, maxLen=dataseries.DEFAULT_MAX_LEN,
                 marketOptions={'assetType': 'STK', 'currency': 'GBP', 'routing': 'SMART'}, warmupBars=0, debug=False):
        ...
        for tickId in range(len(self.__instruments)):
            contractDict[tickId] = (
            self.__instruments[tickId], self.__marketOptions['assetType'], self.__marketOptions['routing'],
            self.__marketOptions['currency'], '', 0.0, '')
        ...
        self.__stockFinishedWarmup = dict((el, False) for el in
                                          self.__instruments)  # has this stock finished warming up? create dict keyed by stock and set to false
        if warmupBars > 0:
            if warmupBars > 200:
    ...
    def __build_bar(self, barMsg, identifier, frequency):
        ...
        try:
            ...
            if len(
                    barMsg.date) == 8:  # it's not a unix timestamp it's something like 20150812 (YYYYMMDD) which means this was a daily bar
                date = datetime.datetime.strptime(barMsg.date, '%Y%m%d')

                date = date + offset
                date = tz.localize(date)
                ts = int((date - datetime.datetime(1970, 1, 1,
                                                   tzinfo=pytz.utc)).total_seconds())  # probably going to have timezone issues

            else:
                ts = int(barMsg.date)
            startDateTime = dt.localize(datetime.datetime.fromtimestamp(ts, tz), tz)
            self.__currentBarStamp = ts
            return bar.BasicBar(startDateTime, float(barMsg.open), float(barMsg.high), float(barMsg.low),
                                float(barMsg.close), int(barMsg.volume), None, frequency)
        except:
            return None
    ...
    def __historicalBarsHandler(self, msg):
        ...
        if self.__inWarmup:
            ...
            if finishedWarmup:
                ...
                print
                "Sleeping for %d seconds for next bar" % delay
                self.__timer = threading.Timer(delay, self.__requestBars)
                self.__timer.daemon = True
                self.__timer.start()
